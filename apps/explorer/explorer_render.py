...
class ExplorerRender:
    ...
    Handles layout rendering, sidebar navigation, folder grid view,
    breadcrumb navigation, and touch/trackpad drag-and-drop overlays.
    """
    ...
    def draw(self, renderer):
        ...
        try:
            ...
            font_greeting = pygame.font.SysFont(
                "Segoe UI", max(18, int(24 * scale)), bold=True
            )
            font_tooltip = pygame.font.SysFont(
            ...
            left_panel_rect = pygame.Rect(
                16,
                self.TITLEBAR_HEIGHT + 10,
                left_w,
                wh - self.TITLEBAR_HEIGHT - bottom_margin - 10,
            )
            center_strip_rect = pygame.Rect(
                left_panel_rect.right + 10,
                self.TITLEBAR_HEIGHT + 10,
                center_w,
                wh - self.TITLEBAR_HEIGHT - bottom_margin - 10,
            )
            ...
            self._draw_center_strip(
                window_surf, center_strip_rect, rel_x, rel_y, scale
            )
            self._draw_right_content(
                window_surf,
                content_x,
                content_w,
                content_h,
            ...
            if getattr(self, "dragging", False) and getattr(
                ...
                self._draw_drag_ghost(surface, mx, my, scale)
    ...
    def _draw_drag_ghost(self, surface, mx, my, scale):
        """Renders semi-transparent item image directly under touch/cursor during drag."""
        item = self.dragging_item
        item_name = getattr(item, "name", "Item")
        is_folder = self._is_item_folder(item)

        icon_sz = max(32, int(40 * scale))
        card_sz = icon_sz + 20
        ...
        icon = (
            self._get_scaled_icon("folder", icon_sz)
            if is_folder
            else self._get_file_icon(item_name, icon_sz)
        )
        if icon:
    ...
    def _get_scaled_icon(
        self, key: str, target_size: int
    ) -> pygame.Surface | None:
    ...
    def _draw_left_panel(self, surface, rect, rel_x, rel_y, font_main, scale):
        ...
        pygame.draw.rect(
            surface, (255, 255, 255, 30), rect, border_radius=18, width=1
        )
        ...
        for name, icon_path in self.sidebar_items:
            ...
            if is_selected:
                pygame.draw.rect(
                    surface, (45, 45, 48, 255), item_rect, border_radius=10
                )
                pygame.draw.rect(
                    surface,
                    (120, 180, 255),
                    (item_rect.x + 2, item_rect.y + 4, 3, item_h - 8),
                    border_radius=2,
                )
                text_color = (255, 255, 255)
            elif is_hovered:
                pygame.draw.rect(
                    surface, (0, 0, 0, 60), item_rect, border_radius=10
                )
                text_color = (255, 255, 255)
            else:
                text_color = (65, 65, 70)
            ...
            icon_key = icon_map.get(name, "folder")
            icon = self._get_scaled_icon(icon_key, icon_sz) or self._get_scaled_icon(
                "folder", icon_sz
            )
            ...
            if icon:
                surface.blit(
                    ...
                    (
                        item_rect.x + 8,
                        item_rect.centery - icon.get_height() // 2,
                    ),
                )
    ...
    def _draw_center_strip(self, surface, rect, rel_x, rel_y, scale):
        ...
        for key in center_items:
            base_ring_rect = pygame.Rect(
                rect.centerx - ring_size // 2, y, ring_size, ring_size
            )
            is_hovered = base_ring_rect.collidepoint(rel_x, rel_y)

            current_ring_size = (
                int(ring_size * 1.15) if is_hovered else ring_size
            )
            ring_rect = pygame.Rect(
                rect.centerx - current_ring_size // 2,
            ...
            ring_border = (
                (255, 255, 255, 190) if is_hovered else (255, 255, 255, 70)
            )

            circle_radius = current_ring_size // 2
            pygame.draw.rect(
                surface, ring_bg, ring_rect, border_radius=circle_radius
            )
            pygame.draw.rect(
                surface,
                ring_border,
            ...
            if img:
                surface.blit(
                    img,
                    (
                        ring_rect.centerx - (img.get_width() // 2),
    ...
    def _draw_plus_button(self, surface, rect, rel_x, rel_y, scale):
        ...
        plus_base_rect = pygame.Rect(
            rect.centerx - plus_ring_sz // 2,
        ...
        curr_plus_ring_sz = (
            int(plus_ring_sz * 1.15) if is_plus_hovered else plus_ring_sz
        )

        plus_ring_rect = pygame.Rect(
            rect.centerx - curr_plus_ring_sz // 2,
        ...
        plus_border = (
            (255, 255, 255, 190) if is_plus_hovered else (255, 255, 255, 70)
        )
        plus_radius = curr_plus_ring_sz // 2

        pygame.draw.rect(
            surface, plus_bg, plus_ring_rect, border_radius=plus_radius
        )
        pygame.draw.rect(
            surface,
            plus_border,
        ...
        if plus_img:
            surface.blit(
                plus_img,
                (
                    plus_ring_rect.centerx - (plus_img.get_width() // 2),
    ...
    def _draw_right_content(
        ...
        self._draw_search_bar(
            surface, content_x, top_y, content_w, rel_x, rel_y, font_small, scale
        )
        self._draw_status_icons(surface, content_x, top_y, content_w, scale)

        grid_start_y = (
            top_y + self._get_search_bar_height(scale) + int(16 * scale)
        )
    ...
    def _draw_search_bar(
        self, surface, content_x, top_y, content_w, rel_x, rel_y, font_small, scale
    ):
        ...
        pygame.draw.rect(
            surface, search_bg, search_rect, border_radius=search_h // 2
        )

        base_icon_sz = max(12, int(16 * scale))
        search_icon_sz = int(
            base_icon_sz + (base_icon_sz * 0.25) * self.search_progress
        )
        ...
        if s_icon:
            ...
            s_icon_x = int(
                start_icon_x
                + (target_icon_x - start_icon_x) * self.search_progress
            )
            ...
            if icon:
                surface.blit(
                    ...
                    (
                        tr_x,
                        top_y
                        + (self._get_search_bar_height(scale) // 2)
                        - (status_icon_sz // 2),
                    ),
                )
            tr_x += int(26 * scale)
    ...
    def _draw_dashboard(
        ...
        greeting = font_greeting.render(
            f"Hi, {self.user_name}", True, (245, 245, 245)
        )
        surface.blit(
            greeting, (cell_00_rect.x, cell_00_rect.y + int(4 * scale))
        )

        ai_w = min(int(110 * scale), int(col_w * 0.8))
        ai_h = max(26, int(34 * scale))
        ai_box_rect = pygame.Rect(
            cell_00_rect.x,
            cell_00_rect.y + greeting.get_height() + int(12 * scale),
            ai_w,
            ai_h,
        )

        pygame.draw.rect(
            surface, (255, 255, 255, 30), ai_box_rect, border_radius=10
        )
        ...
        if ai_icon:
            surface.blit(
                ai_icon,
                (ai_box_rect.x + 10, ai_box_rect.centery - (ai_icon_sz // 2)),
            )

        cards = [
            pygame.Rect(content_x + col_w + spacing, grid_start_y, col_w, row_h),
            pygame.Rect(
                content_x, grid_start_y + row_h + spacing, col_w, row_h
            ),
            pygame.Rect(
                content_x + col_w + spacing,
                grid_start_y + row_h + spacing,
                col_w,
                row_h,
            ),
            pygame.Rect(
                content_x, grid_start_y + (row_h + spacing) * 2, col_w, row_h
            ),
            pygame.Rect(
                content_x + col_w + spacing,
                grid_start_y + (row_h + spacing) * 2,
                col_w,
                row_h,
            ),
        ]
        ...
        for i, card_rect in enumerate(cards):
            ...
            plus_sz = max(
                14, int(min(render_rect.width, render_rect.height) * 0.35)
            )
    ...
    def _draw_folder_contents(
        ...
        for item in items:
            ...
            icon = (
                self._get_scaled_icon("folder", icon_size)
                if is_folder
                else self._get_file_icon(item_name, icon_size)
            )

            if icon:
                surface.blit(
                    icon, (x + (icon_size - icon.get_width()) // 2, grid_y)
                )

            display_name = (
                item_name[:15] + "..." if len(item_name) > 15 else item_name
            )
            txt = font_small.render(display_name, True, (200, 200, 200))
            surface.blit(
                txt,
                (
                    x + (icon_size - txt.get_width()) // 2,
                    grid_y + icon_size + 4,
                ),
            )

            if self.selected_item == item and not is_drop_target:
                pygame.draw.rect(
                    surface,
                    (120, 180, 255, 50),
                    item_rect,
    ...
    def _get_file_icon(self, filename, target_size):
        ...
        return self._get_scaled_icon(icon_key, target_size) or self._get_scaled_icon(
