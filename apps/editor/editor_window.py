...
Text Editor Application Window
"""
...
class EditorWindow(Window):
    """Main Text Editor window handling file editing, buffer state, and rendering."""

    TITLEBAR_HEIGHT = 40

    def __init__(self, title="Text Editor - LibreOffice Writer", width=850, height=650, name="Text Editor") -> None:
        super().__init__(title=title, width=width, height=height, name=name)
        ...
        self.file_name: str = "Untitled.txt"
        self.is_dirty: bool = False
        self.lines: list[str] = [""]
        self.visual_lines: list[dict] = []
        ...
        self._target_cursor_x: float = 0.0

        # Viewport & Dimensions
        self.scroll_y: int = 0
        self.page_padding_x: int = 20
        self.page_padding_y: int = 15
        self.line_height: int = 26
        self.status_bar_height: int = 24

        self._last_width: int = -1
        self._needs_layout_update: bool = True
    ...
    def _load_icons(self) -> None:
        size = 24
        self.raw_icons = {
            "save": IconManager.get("editor/save", size),
            "new": IconManager.get("editor/new", size),
            "document": IconManager.get("files/text", size),
        }
    ...
    def _handle_keyboard(self, event) -> None:
        ...
        if key == pygame.K_BACKSPACE:
            ...
            if not (mods & pygame.KMOD_CTRL or mods & pygame.KMOD_ALT):
    ...
    def _handle_mouse(self, event) -> None:
        ...
        client = pygame.Rect(wx + 2, wy + self.TITLEBAR_HEIGHT, ww - 4, wh - (self.TITLEBAR_HEIGHT + 2))
        page_rect = self._get_page_rect(client)

        pos = getattr(event, "pos", pygame.mouse.get_pos())

        if page_rect.collidepoint(pos):
            rel_x = max(0, pos[0] - (page_rect.x + self.page_padding_x))
            rel_y = pos[1] - (page_rect.y + self.page_padding_y)
            ...
            if self.visual_lines:
                ...
                self.cursor_col = (
                    ...
                    + self._get_col_from_x_offset(target_vline["text"], rel_x)
                )
    ...
    def _delete_backspace(self) -> None:
    ...
    def _move_cursor(self, key: int) -> None:
        if key == pygame.K_LEFT:
            ...
            if 0 <= target_v_idx < len(self.visual_lines):
                ...
                self.cursor_col = (
                    target_vline["col_start"]
                    + self._get_col_from_x_offset(
                        target_vline["text"], self._target_cursor_x
                    )
                )

        self._reset_cursor_blink()

    def _update_target_x(self) -> None:
        self._update_layout()
        _, x_off = self._get_cursor_visual_info()
        self._target_cursor_x = x_off
    ...
    def _get_cursor_visual_info(self) -> tuple[int, int]:
        ...
        for i, vline in enumerate(self.visual_lines):
            ...
            is_last_chunk = True
            if (
                i + 1 < len(self.visual_lines)
                and self.visual_lines[i + 1]["row"] == self.cursor_row
            ):
                is_last_chunk = False
    ...
    def _get_col_from_x_offset(self, text: str, target_x: float) -> int:
    ...
    def _ensure_cursor_visible(self) -> None:
        v_idx, _ = self._get_cursor_visual_info()
        client_height = self.transform.size.height - (self.TITLEBAR_HEIGHT + 24)
        visible_lines_count = max(
            1, (client_height - (self.page_padding_y * 2)) // self.line_height
        )
    ...
    def _draw_workspace(
        self, surface: pygame.Surface, client: pygame.Rect, page_rect: pygame.Rect
    ) -> None:
    ...
    def _draw_document_content(
        self, surface: pygame.Surface, page_rect: pygame.Rect
    ) -> None:
        visible_lines_count = max(
            1, (page_rect.height - (self.page_padding_y * 2)) // self.line_height
        )
        start_index = max(0, self.scroll_y)
        end_index = min(
            len(self.visual_lines), start_index + visible_lines_count + 1
        )
        ...
        for idx in range(start_index, end_index):
            ...
            if idx == cursor_v_idx and self.is_active:
                h_rect = pygame.Rect(
                    base_x - 5,
                    render_y,
                    page_rect.width - (self.page_padding_x * 2) + 10,
                    self.line_height,
                )
        ...
        if (
            self.cursor_visible
            and self.is_active
            and (start_index <= cursor_v_idx < end_index)
        ):
            cursor_y = base_y + (cursor_v_idx - self.scroll_y) * self.line_height
            pygame.draw.rect(
                surface,
                (0, 0, 0),
                (base_x + cursor_x_off, cursor_y + 2, 2, self.line_height - 4),
            )

    def _draw_statusbar(self, surface: pygame.Surface, client: pygame.Rect) -> None:
        status_rect = pygame.Rect(
            client.x,
            client.y + client.height - self.status_bar_height,
            client.width,
